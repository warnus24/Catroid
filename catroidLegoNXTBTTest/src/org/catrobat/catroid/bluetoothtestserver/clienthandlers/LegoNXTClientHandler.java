...
public class LegoNXTClientHandler extends BTClientHandler {
	...
	public void handle(InputStream inStream, OutputStream outStream)
		...
		while ((firstLenByte = bReader.read()) != -1) {
			...
			BTServer.writeMessage("Received message, length (byte): "
					+ messageLength + "\n");
			char[] buf = new char[messageLength];
			byte[] reply = (byte[]) null;
			bReader.read(buf);

			if ((messageLength == 3) && (buf[0] == DIRECT_COMMAND_REPLY)
					&& (buf[1] == '\006')) {
	...
	private byte[] getLegoNXTReplyMessage(char[] lastMessage) {
