...
public final class ArduinoError
{
	private ArduinoError() {}

	private static final String TAG = ArduinoError.class.getSimpleName();
	public static void checkForError(ArduinoDataReply reply, int expectedLength) {
		if (reply.hasError()) {
			throw new Exception(ErrorCode.getTypeByValue(reply.getStatusByte()),
					ArduinoCommandByte.getTypeByValue(reply.getCommandByte()));
		}

		if (reply.getLength() != expectedLength) {
			throw new Exception(ErrorCode.WrongNumberOfBytes,
					ArduinoCommandByte.getTypeByValue(reply.getCommandByte()));
		}
	}
