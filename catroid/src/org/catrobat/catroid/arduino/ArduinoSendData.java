...
public class ArduinoSendData implements Send {

	private static final String TAG = ArduinoSendData.class.getSimpleName();

	private int port;
	private ArduinoConnection connection;

	public ArduinoSendData(int port, ArduinoConnection connection) {
		this.port = port;
	...
	private void trySetOutputState(OutputState state, boolean reply) {
		...
		Command command = new Command(ArduinoCommandType.DIRECT_COMMAND, ArduinoCommandByte.SET_OUTPUT_STATE, false);
